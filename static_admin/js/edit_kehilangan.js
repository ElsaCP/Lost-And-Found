document.addEventListener("DOMContentLoaded", () => {
  // ====== BAGIAN 1: Dropdown Terminal, Tempat, dan Lokasi Lain ======
  ...
  if (terminalSelect && tempatSelect && lokasiInput) {
    ...
    function updateLokasi() {
      ...
      lokasiInput.value = [terminal, tempat === "Lainnya" ? lainnya : tempat]
        .filter(Boolean)
        .join(" - ");
    }
    ...
    terminalSelect.addEventListener("change", () => {
      ...
      if (tempatData[terminal]) {
        tempatData[terminal].forEach((tempat) => {
          const opt = document.createElement("option");
          opt.value = tempat;
          opt.textContent = tempat;
    ...
    if (terminalDB) {
      ...
      if (tempatData[terminalDB]) {
        tempatData[terminalDB].forEach((tempat) => {
          const opt = document.createElement("option");
          opt.value = tempat;
          opt.textContent = tempat;
          tempatSelect.appendChild(opt);
